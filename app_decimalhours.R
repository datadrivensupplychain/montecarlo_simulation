    ...
    sidebarLayout(
        sidebarPanel(
            ...
            numericInput("bobdeparttime",
                         ...
                         value = 17.5,
                         step=0.25),
            numericInput("ralphcommutemean",
                         "Ralph Average Commute:",
                         min=1,
                         value=30),
            numericInput("ralphcommutesd",
                         "Ralph Commute Standard Deviation:",
                         min=1,
                         value=10),
            numericInput("bobcommutemean",
                         "Bob Average Commute:",
                         min=1,
                         value=30),
            numericInput("bobcommutesd",
                         "Bob Commute Standard Deviation:",
                         min=1,
                         value=10),
...
server <- function(input, output) {
    ...
    outdf_r <- reactive({
        ...
        ralph_commute_duration_minutes <-   rnorm(n=input$iterations, #removed pmax 15
                                                       mean=input$ralphcommutemean,
                                                       sd=input$ralphcommutesd)
        bob_depart_time <-  rep(input$bobdeparttime,input$iterations)
        bob_commute_duration_minutes <- rnorm(n=input$iterations,
                                                    mean=input$bobcommutemean,
                                                    sd=input$bobcommutesd) #removed pmax 15
